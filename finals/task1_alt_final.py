...
def train_and_evaluate_model(
    model_class, num_hidden_units=32, lr=1e-5, batch_size=32
):
...
if 1.1 in TASKS:
    ...
    for num_hidden_units in hidden_units_list:
        ...
        plt.figure(figsize=(12, 6))
        plt.plot(train_losses, label=f"Hidden Units: {num_hidden_units}")
        plt.xlabel("Epochs")
        plt.ylabel("Training Loss")
        plt.title("Training Loss for Different Hidden Units in ShallowCorrectorMLP")
        plt.legend()
        plt.savefig(f"{FIG_DIR}/training_losses.{EXT}")
        plt.close("all")
...
if 1.3 in TASKS:
    print("=" * 20, "Task 1.3", "=" * 20)
    results = {}
    for lr in learning_rates:
        results[lr] = {}
        for model_class in model_classes:
            class_name = model_class.__name__
            results[lr][class_name] = train_and_evaluate_model(
                model_class, num_hidden_units=32, lr=lr
            )
        FIG_DIR = os.path.join(
            FIG_BASE_DIR, "task1_3_alt", f"learning_rate_{lr}"
        )
        os.makedirs(FIG_DIR, exist_ok=True)
        plt.figure(figsize=(12, 6))
        plt.plot(t, q_target, "r-", label="Target")
    ...
    for i, model_class in enumerate(model_classes):
        ...
        for lr in learning_rates:
            final_loss[i].append(
                results[lr][model_class.__name__][0][-1]
            )
    fix, ax = plt.subplots(figsize=(12, 6))
    im = ax.imshow(final_loss)
    ...
    ax.set_ylabel("Model")
    ax.set_title("Final Loss for Different Models and Learning Rates")
    plt.colorbar(im)
    plt.savefig(f"{FIG_BASE_DIR}/task1_3_alt/final_loss.{EXT}")
    plt.close("all")

if 1.4 in TASKS:
    print("=" * 20, "Task 1.4", "=" * 20)
    results = {}
    for batch_size in batch_sizes:
        results[batch_size] = {}
        for model_class in model_classes:
            class_name = model_class.__name__
            results[batch_size][class_name] = train_and_evaluate_model(
                model_class, num_hidden_units=32, lr=1e-1
            )
        FIG_DIR = os.path.join(
            FIG_BASE_DIR, "task1_4_alt", f"batch_size{batch_size}"
        )
        os.makedirs(FIG_DIR, exist_ok=True)
        plt.figure(figsize=(12, 6))
        plt.plot(t, q_target, "r-", label="Target")
    ...
    for i, model_class in enumerate(model_classes):
        ...
        for batch_size in batch_sizes:
            final_loss[i].append(
                results[batch_size][model_class.__name__][0][-1]
            )
    fix, ax = plt.subplots(figsize=(12, 6))
    im = ax.imshow(final_loss)
    ...
    ax.set_ylabel("Model")
    ax.set_title("Final Loss for Different Models and Batch Sizes")
    plt.colorbar(im)
    plt.savefig(f"{FIG_BASE_DIR}/task1_4_alt/final_loss.{EXT}")
    plt.close("all")
